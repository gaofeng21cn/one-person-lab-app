...
function validateIdentity(identity: Identity): string {
  ...
  invariant(shaPattern.test(identity.frameworkRef), 'Studio checkpoint Framework ref must be exact lowercase SHA.');
...
function validateSupportingEvidence(root: string, identity: Identity, version: string, files: CheckpointFile[]): void {
  ...
  invariant(source.schema === 'opl_studio_protected_release_admission.v2', 'Studio source admission schema is invalid.');
  ...
  invariant(source.framework_bootstrap?.framework_ref === identity.frameworkRef, 'Studio source admission Framework ref does not match.');
...
export function sealStudioReleaseCheckpoint(root: string, identity: Identity) {
  ...
  const checkpoint = {
    schema: 'opl_studio_signed_notarized_checkpoint.v2',
...
export function validateStudioReleaseCheckpoint(root: string, identity: Identity) {
  ...
  invariant(exactKeys(checkpoint, ['schema', 'status', 'authority_owner', 'app_executor_sha', 'framework_ref', 'source', 'files']), 'Studio checkpoint shape is invalid.');
  invariant(checkpoint.schema === 'opl_studio_signed_notarized_checkpoint.v2', 'Studio checkpoint schema is invalid.');
  ...
  invariant(checkpoint.framework_ref === identity.frameworkRef, 'Studio checkpoint Framework ref does not match.');
...
export function sealStudioReleaseQualification(input: QualificationIdentity) {
  ...
  const manifest = {
    schema: 'opl_studio_prepublication_qualification.v2',
    status: 'qualified',
    authority_owner: 'one-person-lab-app',
...
export function validateStudioReleaseQualification(input: QualificationIdentity) {
  ...
  invariant(exactKeys(manifest, [
    'schema', 'status', 'authority_owner', 'app_executor_sha', 'framework_ref', 'source',
    'checkpoint_manifest_sha256', 'receipt', 'public_feed_pending',
  ]), 'Studio qualification manifest shape is invalid.');
  invariant(manifest.schema === 'opl_studio_prepublication_qualification.v2', 'Studio qualification manifest schema is invalid.');
  ...
  invariant(manifest.framework_ref === input.frameworkRef, 'Studio qualification Framework ref does not match.');
...
function runCli(argv: string[]): void {
  ...
  for (const option of ['app-ref', 'studio-sha', 'studio-tree', 'studio-tag', 'framework-ref'] as const) {
