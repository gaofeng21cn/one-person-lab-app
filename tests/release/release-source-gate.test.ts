...
function runner(overrides: Record<string, { status: number; stdout?: string; stderr?: string }> = {}): CommandRunner {
  return (command, args, commandOptions) => {
    ...
    if (
      command === process.execPath
      && args.join(' ') === '--experimental-strip-types scripts/run-active-shell-tests.ts --project all --max-workers 1'
      && commandOptions.cwd === repoRoot
    ) {
...
test('release source gate does not depend on the repository immutable Releases setting', () => {
  const calls: string[] = [];
  const baseRunner = runner();
  const report = buildReleaseSourceGateReport(
    options(),
  ...
  assert.equal('immutable_release_capability' in report, false);
  assert.equal(report.checks.some((check) => check.id === 'github_immutable_release_capability'), false);
  assert.equal(calls.some((call) => call.includes('/immutable-releases')), false);
});
...
test('release source gate blocks pre-admission when required shell node/dom tests are not enabled', () => {
  ...
  assert.equal(
    ...
    'node --experimental-strip-types scripts/run-active-shell-tests.ts --project all --max-workers 1',
  );
...
test('release source gate fails active shell node/dom regressions before expensive release work', () => {
  const report = buildReleaseSourceGateReport(
    ...
    runner({
      [`${repoRoot} $ ${process.execPath} --experimental-strip-types scripts/run-active-shell-tests.ts --project all --max-workers 1`]: {
        status: 1,
