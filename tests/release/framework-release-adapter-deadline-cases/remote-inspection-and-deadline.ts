...
for (const status of ['403', '404']) {
  test(`explicit GitHub ${status} permission rejection does not become an unknown mutation`, () => {
    ...
    const result = applyPublishPlan({
      ...
      run(_command, args) {
        ...
        if (args[3] === `repos/${repo}/releases`) {
          mutations += 1;
          return { status: 1, stdout: JSON.stringify({ status, message: 'Not Found',
            documentation_url: 'https://docs.github.com/rest/releases/releases#create-a-release' }), stderr: `HTTP ${status}` };
        }
        throw new Error(`Unexpected gh call: ${args.join(' ')}`);
      },
    });
