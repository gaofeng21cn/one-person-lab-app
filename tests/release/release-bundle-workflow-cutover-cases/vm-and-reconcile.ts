...
test('first-run VM validates Studio production Runtime refresh before writing qualification evidence', () => {
  ...
  assert.equal(stepIndex('Qualify public Aion Stable upgrades to the exact sealed Studio candidate'), -1);
  const receiptIndex = stepIndex('Write exact-artifact qualification receipt');
  assert.ok(validationIndex < receiptIndex);
  assert.doesNotMatch(String(steps[receiptIndex].run), /steps\.legacy_upgrade\.outcome/);
  assert.match(String(steps[receiptIndex].run), /steps\.vm_smoke\.outcome/);
