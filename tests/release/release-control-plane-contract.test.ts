...
test('release platform contract keeps the primary Stable platform separate from same-tag Desktop additions', () => {
  ...
  assert.deepEqual(matrix.desktop_platform_additive_follower.stable_additive_repair, {
    ...
    installer_runtime_identity:
      'component_manifest_initial_installer_identity_plus_one_complete_unforked_digest_and_size_chain_across_all_public_additive_repair_receipts_to_the_current_release_asset',
  ...
  const missingCoreMac = structuredClone(release);
  missingCoreMac.release_validation_profiles.stable.required_lanes =
    missingCoreMac.release_validation_profiles.stable.required_lanes.filter(
      (lane: string) => lane !== 'standard_macos_arm64_build',
    );
  assert.throws(
    () => validateAppGuiProductContract(gui, missingCoreMac, installExposure),
