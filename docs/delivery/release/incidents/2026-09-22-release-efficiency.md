...
在同一宿主使用已公开 Full DMG 中同一 App、相同 staging 进行有界对照，源 DMG SHA-256 为 `c71988f011929f666ccad8092690788d7d390dc71869c4ecc5f4185347919a30`。
...
两份输出的 21,210 个文件、权限和软链与源 App 一致，严格 codesign 验证通过；没有删除 payload。默认采用 UDZO 7，保留 ULMO 显式选择。该样本压缩耗时减少约 77.7%，代价是体积增加约 23.6%，超过现有 700 MB 警告线但仍低于 750 MB 复核线；预算阈值保持不变。

这仅证明本机同内容的压缩差异，不等于下一次 CI 或端到端发布可缩短同样比例。签名、公证、普通账号首次登录、clean-VM、公开资产摘要和渠道指针回读继续执行；下一次正式发布应按同一计时口径记录实际收益。本轮修复不重发已公开版本。
