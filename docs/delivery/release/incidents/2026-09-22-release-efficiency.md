...
两份输出的 21,210 个文件、权限和软链与源 App 一致，内嵌 App 的严格 codesign 验证通过；没有删除 payload。对照 DMG 容器未重新签名、公证或发布。默认采用 UDZO 7，保留 ULMO 显式选择。该样本压缩耗时减少约 77.7%，代价是体积增加约 23.6%，超过现有 700 MB 警告线但仍低于 750 MB 复核线；预算阈值保持不变。
