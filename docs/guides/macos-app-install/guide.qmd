...
![首次启动检查工作目录、本机助手与模型访问](screenshots/04-first-run-checking.png){width=96% fig-align="center"}
...
- **刷新结果**：完成配置后点击“刷新状态”；模型选择在同组的“模型”页面。

![在账户与模型中登录或配置模型访问](screenshots/03-codex-config-needed.png){width=96% fig-align="center"}

OPL Gateway 的服务主机是 `gateway.medopl.com`。新配置由 App 按服务合同处理，无需手工猜填地址。本机已有配置会保留其关联；请勿把密码或完整 API Key 放进聊天、截图或代码仓库。

## 5. 开始第一项工作

关闭设置，点击“新建任务”，选择工作目录，在输入框描述目标。可通过输入框上方的智能体选择器使用当前已安装的能力，也可从附件入口添加文件或 Skill；入口随可用能力变化。

![在新版工作台选择工作区并描述目标](screenshots/05-opl-ready-dynamic-entries.png){width=96% fig-align="center"}

第一条任务建议同时说明目标和材料位置，例如：

> 请整理当前项目中已经脱敏的研究材料，列出已有证据、缺失资料和下一步工作。

涉及患者或敏感研究数据时，应先完成脱敏，并遵守所在机构的数据管理和伦理要求。

## 6. 查看状态与后续更新

“设置 → 概览”汇总账户与当前运行状态。详细的“功能诊断”默认收起，需要排查时再展开；未使用或未配置的功能不一定有完整状态。

![概览采用行式布局，功能诊断按需展开](screenshots/08-opl-runtime-status.png){width=96% fig-align="center"}
